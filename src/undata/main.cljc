(ns undata.main
  "Kotodama WASM entrypoint for the Undata clean-room actor (L4) — Clojure port.
  ...
  facts (`undata.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "undata")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"undata.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "undata-compat" :tier tier :entities entities} 200])
...
(defn start! [] :undata-compat/ready)
